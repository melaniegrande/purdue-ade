function [ conn,run ] = Ground_contact_stk_rev2( startdate,enddate )
...
% % EDIT BY ISAAC DROLL:
% .csv input modified to allow for avg/min/max cases
global sim_case
...
elseif strcmp(sim_case,'Min')
    Cal = csvread('CalPolycontact - Min.csv',1,0); %takes excel data calpoly
    Purdue = csvread('Purduecontact - Min.csv',1,0); %takes excel data purdue
    ASU = csvread('ASUcontact - Min.csv',1,0); %takes excel data
    Tech = csvread('GaTechcontact - Min.csv',1,0); %takes excel data
end
julianseconds = (((1/24)/60)/60); %creates seconds in julian time
step = julianseconds*30; % 30 second step
